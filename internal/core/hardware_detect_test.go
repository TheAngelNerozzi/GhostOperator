...
func TestDetectHardwareProfile_PopulatesAllFields(t *testing.T) {
	...
	// On any real machine, these should be non-zero
	assert.Greater(t, profile.TotalRAMBytes, uint64(0), "TotalRAMBytes should be > 0")
	assert.Greater(t, profile.FreeRAMBytes, uint64(0), "FreeRAMBytes should be > 0")

...
func TestPhantomPulse_AdaptiveResize(t *testing.T) {
        ...
        assert.Equal(t, 1280, bounds.Dx())
        assert.Equal(t, 720, bounds.Dy())
}
...
func TestPhantomPulse_AdaptiveResize_Fallback(t *testing.T) {
        ...
        // In fallback mode, should resize to 960x540
        assert.Equal(t, 960, bounds.Dx())
        assert.Equal(t, 540, bounds.Dy())
}
